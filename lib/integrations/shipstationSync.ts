...
function detectChannel(o: ShipStationOrder): { channel: string; source: string } {
  const src = `${o.orderSource ?? ""} ${o.advancedOptions?.source ?? ""} ${o.customerUsername ?? ""} ${o.orderNumber ?? ""}`.toLowerCase();
  if (src.includes("ebay") || src.includes("e bay")) return { channel: "ebay", source: "shipstation" };
  ...
  if (src.includes("woocommerce") || src.includes("woo")) return { channel: "woocommerce", source: "shipstation" };
...
export type ShipStationSyncResult = {
  ...
  pages: number;
  ebay_fetched: number;
  ebay_upserted: number;
  ebay_items_written: number;
};
...
export async function syncShipStationOrders(opts?: {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    for (const o of orders) {
      ...
      if (orderErr || !orderRow?.id) {
        ...
        upserted += 1;
        if (channel === "ebay") ebayUpserted += 1;
        await writeOrderItems(sb, orgId, orderId, o);
        itemsWritten += (o.items?.length ?? 0);
        if (channel === "ebay") ebayItemsWritten += (o.items?.length ?? 0);
        continue;
      }
  ...
  return {
    ...
    pages: pagesUsed,
    ebay_fetched: ebayFetched,
    ebay_upserted: ebayUpserted,
    ebay_items_written: ebayItemsWritten,
