...
function normalizeRedirectTo(raw: string | null) {
  const fallback = "/account";
  if (!raw) return fallback;

  // Handle double-encoding safely
  const d1 = safeDecode(raw);
  const d2 = safeDecode(d1);
  const candidate = (d2 || d1).trim();
...
export default function LoginClient() {
  ...
  // Support both ?next= and ?redirectTo=
  const redirectTo = useMemo(() => {
    const raw = sp.get("next") ?? sp.get("redirectTo");
    return normalizeRedirectTo(raw);
  }, [sp]);
  ...
  // Preserve redirect intent when sending users to signup
  const signupHref = useMemo(() => {
    const qs = new URLSearchParams({ next: redirectTo });
